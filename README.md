...
|`cal:collaboration`|This container holds information about the collaboration</br> Attribute “id” **OPTIONAL** is only needed if two (2) or more collaborations publish together. Typically, it is a sequential number, starting at “c1”. It is used to denote the collaboration in this particular author.xml file for the purpose of attaching authors and/or institutes to the collaboration in a multi-collaboration environment. NOTE: For bizarre historical reasons that few people remember, XML requires that an attribute value declared as being of type ID has the same syntax as an XML name – so it cannot start with a digit..</br>
