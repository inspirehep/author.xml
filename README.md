...
|`cal:publicationReference`|an internal report number an arXiv number **or** a collaboration’s internal document number **or** an ISBN; a DOI; a persistant web destination; anything that identifies the referenced document. If no immediate identifier, the title can be used|Required|
